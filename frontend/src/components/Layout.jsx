...
import ProfileModal from './ProfileModal'; // Đảm bảo đã tạo ProfileModal.jsx
import {
  Home,
  Users,
  FileText,
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-row justify-start">
      ...
      >
        ...
        {/* User info - Clickable */}
        <div className="px-6 py-4 border-b bg-primary-50">
          <button
            onClick={() => setIsProfileModalOpen(true)}
            className="w-full text-left focus:outline-none hover:bg-primary-100 transition-colors rounded-lg p-2 -m-2"
          >
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-900">{user.name}</p>
                <p className="text-xs text-gray-500">
                  {user.role === 'BQL' ? 'Ban quản lý' : `Căn hộ ${user.apartmentNumber}`}
                </p>
              </div>
