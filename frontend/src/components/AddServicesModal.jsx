...
//     setError(null);
//     try {
//       const servicesData = await contractsService.getAvailableServices();
//       const optionalServices = servicesData.filter(s => 
//         s.type === 'optional' && 
//         s.isActive && 
//         !excludeServiceIds.includes(s.id)
//       );
//       setServices(optionalServices);
//     } catch (err) {
...
const AddServicesModal = ({ isOpen, onClose, onServicesSelected, excludeServiceIds = [] }) => {
  const [services, setServices] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(null);
  const [searchTerm, setSearchTerm] = useState('');
  ...
  const loadServices = async () => {
    ...
    try {
      const servicesData = await contractsService.getAvailableServices();
      const optionalServices = servicesData.filter(s => s.type === 'optional' && s.isActive && !excludeServiceIds.includes(s.id));
      setServices(optionalServices);
    } catch (err) {
      setError(handleApiError(err));
    } finally {
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-xl p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {isLoading ? (
            ...
            <ErrorFallback error={error} onRetry={handleRetry} message="Không thể tải danh sách dịch vụ" />
          ) : (
            <>
              <div className="space-y-4 mb-6 max-h-96 overflow-y-auto">
                {filteredServices.map((service) => (
                  <div
                    ...
                    className={cn(
                      'border rounded-lg p-4 transition-all cursor-pointer',
                      selectedServices.has(service.id) ? 'border-primary bg-primary-50' : 'border-gray-200 hover:border-gray-300'
                    )}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-start space-x-3">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center space-x-4">
                              <div className="flex items-center text-sm text-gray-600">
                                <DollarSign className="h-4 w-4 mr-1" />
                                <span className="font-medium">{service.price.toLocaleString('vi-VN')} {service.unit}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center space-x-2 ml-4">
                        <button onClick={() => handleViewServiceDetails(service)} className="text-blue-600 hover:text-blue-800 p-1 rounded" title="Xem chi tiết điều khoản">
                          <HelpCircle className="h-4 w-4" />
                        </button>

                        <button
                          onClick={() => handleServiceToggle(service.id)}
                          className={cn('px-3 py-1 rounded-lg text-sm font-medium transition-colors', selectedServices.has(service.id) ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-primary text-white hover:bg-primary-600')}
                        >
                          {selectedServices.has(service.id) ? (
                            <>
                              <X className="h-3 w-3 mr-1 inline" />
                ...
                {filteredServices.length === 0 && !isLoading && (
                  <div className="text-center py-8">
                    <Tag className="h-12 w-12 text-gray-300 mx-auto mb-4" />
                    <p className="text-gray-500">{searchTerm ? 'Không tìm thấy dịch vụ phù hợp' : 'Không có dịch vụ nào khả dụng'}</p>
                  </div>
                )}
              </div>
              ...
              {selectedServices.size > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                  <h4 className="font-medium text-blue-900 mb-2">Đã chọn {selectedServices.size} dịch vụ</h4>
                  <div className="flex flex-wrap gap-2">
                    {Array.from(selectedServices).map(serviceId => {
                      const service = services.find(s => s.id === serviceId);
                      return service ? (
                        <span key={serviceId} className="inline-flex items-center px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                          {service.name}
                          <button onClick={() => handleServiceToggle(serviceId)} className="ml-1 text-blue-600 hover:text-blue-800">
                            <X className="h-3 w-3" />
                          </button>
                        </span>
                      ) : null;
